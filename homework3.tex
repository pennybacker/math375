...
\part Using $f(x) = x^2-5$ and $x_0 = 2$, creating a table of values which demonstrates that Halley's method exhibits cubic convergence at simple roots of $f(x)$. The first column contains the iteration number $i$. The second column contains the approximation $x_i$. The third column contains the absolute error $\varepsilon_i$. The fourth column contains the error ratio $\varepsilon_{i+1} / \varepsilon_i^3$. All numerical values should be printed to at least 10 digits. Explain briefly how your data demonstrate cubic convergence.
